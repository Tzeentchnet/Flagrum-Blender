...
def generate_material(context: ImportContext, mesh_data: MeshData) -> Material:
    ...
    needs_scaling = mesh_data.BlenderMaterial.UVScale is not None and (
        mesh_data.BlenderMaterial.UVScale[0] != 1.0
        or (len(mesh_data.BlenderMaterial.UVScale) > 1 and mesh_data.BlenderMaterial.UVScale[1] != 1.0)
    )
    ...
    for t in mesh_data.BlenderMaterial.Textures:
        ...
        if "BASECOLOR0" in texture_slot:
            ...
            if (
                hasattr(mesh_data.BlenderMaterial, "DetailUVScale")
                and mesh_data.BlenderMaterial.DetailUVScale is not None
            ):
