...
from ..utilities.asset_catalog import mark_material_asset
...
class GmtlImporter:
    ...
    def generate_material(self, has_light_map: bool) -> Material:
        ...
        mark_material_asset(material, self.context.model_name)
        ...
        if needs_scaling:
            ...
            uv_scale.inputs[3].default_value[1] = uvscale_buffer[len(uvscale_buffer) - 1]
        ...
        for texture_metadata in gmtl.textures:
            ...
            if "BASECOLOR0" in texture_slot or "BASECOLORTEXTURE0" in texture_slot:
                ...
                if texture_metadata.name.endswith("_nrt") or texture_metadata.name.endswith("_nrt_$h"):
                ...
                elif texture_metadata.name.endswith("_nro") or texture_metadata.name.endswith("_nro_$h"):
                    norm_map = material.node_tree.nodes.new("ShaderNodeNormalMap")
                    material.node_tree.links.new(bsdf.inputs["Normal"], norm_map.outputs["Normal"])
                    rgb = material.node_tree.nodes.new("ShaderNodeRGB")
                    rgb.outputs[0].default_value = (1.0, 1.0, 1.0, 1.0)
                ...
                texture.image.colorspace_settings.name = "Non-Color"
                material.node_tree.links.new(principled_input(bsdf, "specular"), texture.outputs["Color"])
                ...
                or "OPACITYMASKTEXTURE0" in texture_slot
                or "TRANSPARENCYTEXTURE0" in texture_slot
            ):
                texture.image.colorspace_settings.name = "Non-Color"
                ...
                if detail_uvscale_buffer is not None:
                    ...
                    uv_scale.inputs[3].default_value[1] = detail_uvscale_buffer[len(detail_uvscale_buffer) - 1]
